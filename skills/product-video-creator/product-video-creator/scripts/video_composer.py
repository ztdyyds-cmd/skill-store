...
def create_subtitle_clip(subtitle: Dict, video_width: int, video_height: int):
    ...
    try:
        ...
        # 设置位置
        if position == 'top':
            txt_clip = txt_clip.set_position(('center', video_height * 0.1))
        ...
        if has_background:
            ...
            # 设置背景位置
            if position == 'top':
                bg_clip = bg_clip.set_position(('center', video_height * 0.1 - 10))
            elif position == 'center':
...
def create_video_from_images(
    ...
    for i, (img_path, duration, transition) in enumerate(zip(image_paths, durations, transitions)):
        ...
        img_clip = img_clip.set_start(current_time).set_duration(duration)
        ...
        if transition == '淡入' and i == 0:
            ...
            def slide_in_effect(t):
                progress = t / trans_duration if t < trans_duration else 1
                return min(progress, 1)
            img_clip = img_clip.set_position(lambda t: (video_width * (1 - slide_in_effect(t)), 'center'))
        elif transition == '拉':
            # 简化的拉出效果
            def slide_out_effect(t):
                progress = (duration - t) / trans_duration if t > duration - trans_duration else 1
                return max(-progress, 0)
            img_clip = img_clip.set_position(lambda t: (video_width * slide_out_effect(t), 'center'))
        elif transition == '变焦':
            # 变焦效果：缩放图片
            def zoom_effect(t):
                return 1 + 0.1 * np.sin(2 * np.pi * t / duration)
            img_clip = img_clip.resize(lambda t: zoom_effect(t))

        clips.append(img_clip)
        current_time += duration - trans_duration if i < len(image_paths) - 1 else duration
...
def compose_video(
    ...
    # 如果没有提供时长，根据字幕或音频计算
    if not durations and subtitles:
        total_duration = max([s.get('end_time', 0) for s in subtitles])
        print(f"根据字幕计算总时长: {total_duration}秒")
        # 平均分配每个镜头的时长
        avg_duration = total_duration / len(images)
        durations = [avg_duration] * len(images)
        video_clip = create_video_from_images(images, durations, transitions, fps)
    elif not durations:
        print("警告：未提供时长信息，使用默认时长3秒")
...
def main():
    parser = argparse.ArgumentParser(description='商品视频合成工具')
